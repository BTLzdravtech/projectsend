 ...
 */
if(!empty($_GET['user'])) {
    $user = get_client_by_id($_GET['user']);
	/** Add the name of the client to the page's title. */
	if ($user) {
		$page_title = __('Workspaces where','cftp_admin').' '.html_entity_decode($user['name']).' '.__('is member','cftp_admin');
		$member_exists = 1;
...
<?php
	...
	if (isset($found_workspaces)) {
		if ($found_workspaces != '') {
			$cq .= $next_clause. " FIND_IN_SET(id, :workspaces)";
...
?>
	...
	<form action="workspaces.php" name="workspaces_list" method="get" class="form-inline batch_actions">
		...
		<div class="form_actions_count">
			<p><?php _e('Found','cftp_admin'); ?>: <span><?php echo $count_for_pagination; ?> <?php _e('workspaces','cftp_admin'); ?></span></p>
		</div>
