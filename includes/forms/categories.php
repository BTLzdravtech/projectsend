    ...
    <div class="categories_form form-horizontal">
        ...
        <div class="form-group">
            ...
            <div class="col-sm-8 field_wrapper">
                <input type="text" name="category_name" id="category_name" class="form-control required"
                       value="<?php echo (isset($category_name)) ? html_output(stripslashes($category_name)) : ''; ?>"
                       required/>
            </div>
        </div>
        ...
        <div class="form-group">
            <label for="category_description"
                   class="col-sm-4 control-label"><?php _e('Description', 'cftp_admin'); ?></label>
            <div class="col-sm-8">
                <textarea name="category_description" id="category_description" class="form-control"><?php
                    if (!empty($category_description)) {
                        echo html_output($category_description);
                    }
                    ?></textarea>
            </div>
        </div>
        ...
        <div class="inside_form_buttons">
            <button type="submit" name="btn_process" id="btn_process"
                    class="btn btn-wide btn-primary"><?php echo html_output($submit_value); ?></button>
            <?php if ($show_cancel === true) { ?>
                <a href="<?php echo BASE_URI; ?>categories.php"
                   class="btn btn-wide btn-default"><?php _e('Cancel', 'cftp_admin'); ?></a>
            <?php } ?>
        </div>
    </div>
</form>